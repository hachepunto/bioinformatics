...
```
$ sort breastcancer_regulon.sif | less -S
```
...
```
$ cut -f1 breastcancer_regulon.sif | tail  -n +2 | sort | sed -e 's/Female/Mujer/' -e 's/Male/Hombre/' | less
```

``` 
$ cut -f1,4 breastcancer_regulon.sif | tail  -n +2 | sort -u | sed -e 's/AFF4/AFF1/' -e 's/ARNT2/ARNT/' | less
```
...
``` 
$ cut -f1,4 breastcancer_regulon.sif | tail  -n +2 | sort -u | sed -e 's/AFF4/AFF1/' -e 's/ARNT2/ARNT/' > TFs.txt
```
...
```
$ grep "José" TFs.txt | wc -l
```

```
$ grep "José" TFs.txt > pepes.txt
```
...
```
```
$ cat TFs.txt pepes.txt
```

***paste*** (pegar) pega columnas una contigua a la otra

```
$ paste TFs.txt pepes.txt | less
```

```
$ paste TFs.txt TFs.txt | less
```
