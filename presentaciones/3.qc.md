...
```
scp -r alumnoX@10.0.13.97:~/datos.taller/salida_fastqc .
```
...
Para realizar el ejercicio con Fastp vamos a crear un script para ejecutar el comando. 
En la terminal teclea abre el editor de texto llamado nano.
...
Copia todo el texto de abajo y guárdalo en un archivo fastp.sh en la carpeta datos.taller.

```
bin=/bodega/curso_intro_bioinfo/bin/
reads=~/datos.taller/head_1mill_input_forward_R1_001.fastq
mates=~/datos.taller/head_1mill_input_reverse_R2_001.fastq
outdir=~/datos.taller/salida_fastp

mkdir -p ${outdir}
cd ${outdir}

${bin}/fastp -i ${reads} -I ${mates} -o out.R1.fq.gz -O out.R2.fq.gz
```
...
Ya que tenenos el archivo, desde drona, en datos.taller ejecutamos el siguiente comando:
```
...
Descarga la anotación de genes de la página de Ensembl para humanos, [**archivo gtf**](https://ftp.ensembl.org/pub/current_gtf/homo_sapiens/Homo_sapiens.GRCh38.115.chr.gtf.gz), utilizando los comandos que ya conoces obten un archivo que contenga únicamente la columna con el id del transcript, el id del gen, y el nombre del gen para todos los genes en el chromosoma 22. Es importante no tener líneas repetidas.
