...
```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript
```
...
```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9
```
...
```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9 | cut -d';' -f1,3,5
```

Por ahí se colaron algunas líneas que no tienen nombre de gen, entonces para eliminarlas, hacemos lo siguiente

```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9 | cut -d';' -f1,3,5 | grep -w 'gene_name'
```
Finalmente le damos el formato que queremos, de nuevo usando cut

```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9 | cut -d';' -f1,3,5 | grep -w 'gene_name' | cut -d' ' -f2,4,6
```
Ahora borramos las comillas y los ; y reemplazamos los espacios por tabs:

```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9 | cut -d';' -f1,3,5 | grep -w 'gene_name' | cut -d' ' -f2,4,6 | sed 's/"//g' | sed 's/;//' | sed 's/\s/\t/g' 
```

Ahora si, ya tenemos lo que queríamos y lo podemos guardar a un archivo 

```
zcat Homo_sapiens.GRCh38.113.chr.gtf.gz | grep '^22' | grep -w transcript | cut -f9 | cut -d';' -f1,3,5 | grep -w 'gene_name' | cut -d' ' -f2,4,6 | sed 's/"//g' | sed 's/;//' | sed 's/\s/\t/g' | sort -u > geneId_transcriptId_geneName_chr22.txt
```
...
Liga: https://combine-lab.github.io/salmon/

Para empezar a usarlo revisa la sección **Getting Started**.

Salmon ya está instalado en Drona, pero si lo quieres instalar en tu computadora, descarga el archivo con el binario y descomprímelo.
